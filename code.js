...
function extractLayerStructure(node) {
    let layer = {
        id: node.id,
        name: node.name,
        type: node.type,
    ...
    if ("fills" in node && Array.isArray(node.fills) && node.fills.length > 0) {
        layer.fills = node.fills.map((fill) => {
    ...
    if ("strokes" in node && Array.isArray(node.strokes) && node.strokes.length > 0) {
        layer.strokes = node.strokes.map((stroke) => ({
            type: stroke.type,
            color: "color" in stroke ? stroke.color : null,
            thickness: "strokeWeight" in node ? node.strokeWeight : null
        }));
    }
    ...
    if (node.type === "TEXT") {
        ...
        if (typeof textNode.fontName !== "symbol") {
            layer.fontFamily = textNode.fontName.family;
        }
...
figma.ui.onmessage = (msg) => __awaiter(void 0, void 0, void 0, function* () {
    if (msg.type === "getSelectedFrame") {
        ...
        if (selection && selection.type === "FRAME") {
            ...
            const image = yield selection.exportAsync({ format: "PNG" });
